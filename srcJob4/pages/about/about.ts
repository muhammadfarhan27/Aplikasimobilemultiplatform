...
export class AboutPage {
  ...
  constructor(public navCtrl: NavController, public
    navParams: NavParams, private userProvider: UserProvider, 

    private toastController: ToastController) {
     this.model = this.navParams.get('menuAccount');
     if(this.model){
     console.log("userAccount Params available in
    AboutPage!");
     console.log(this.model);
     }else{
     console.log("userAccount Params not Available in
    AboutPage!");
     }
     }
     updateProfile(){
    ...
    !this.model.email || !this.model.affiliation){
     this.showToast("Please fill user profile ) {
  }
  completely!");
 return null;
 }
...
this.userProvider.updateUserAccount(this.model).then(userAcc
ounts => {
